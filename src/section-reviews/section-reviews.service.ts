import { Injectable } from "@nestjs/common";
import { InjectRepository } from "@nestjs/typeorm";
import { LanguageService } from "src/language/language.service";
import { BaseService } from "src/shared/base/base";
import { APIFeaturesService } from "src/shared/filters/filter.service";
import { ICrudService } from "src/shared/interfaces/crud-service.interface";
import { Repository } from "typeorm";
import { CreateSectionReviewsDto, UpdateSectionReviewsDto } from "./dto/create-section-reviews.dto";
import { SectionReviews } from "./section-reviews.entity";

@Injectable()
export class SectionReviewsService
  extends BaseService<SectionReviews, CreateSectionReviewsDto, UpdateSectionReviewsDto>
  implements ICrudService<SectionReviews, CreateSectionReviewsDto, UpdateSectionReviewsDto>
{
  constructor(
    @InjectRepository(SectionReviews)
    repository: Repository<SectionReviews>,
    protected readonly apiFeaturesService: APIFeaturesService,
    protected readonly languageService: LanguageService,
  ) {
    super(repository, apiFeaturesService, languageService);
  }

  // Override findAll to return data without pagination
  public async findAll(filterData: any) {
    return this.findFront({
      query: {
        ...filterData,
        isPagination: "false",
      },
    });
  }

  // Get active section reviews data
  public async getActiveSectionReviews() {
    const sections = await this.findAll({
      is_active: true,
      relations: {
        language: {
          select: ["id", "name"],
        },
      },
    });
    if (sections && sections.data && sections.data.length > 0) {
      return { content: [{ ...sections.data[0], language_id: sections.data[0].language.id }] };
    }
    return { content: null };
  }
}
